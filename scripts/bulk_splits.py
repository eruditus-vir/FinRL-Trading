"""Bulk-fetch split history for the universe.

Step 4 Component 4 (2026-05-07). One call per ticker — `/splits?symbol=X`
returns all-history. UNIQUE(ticker, date) handles re-runs. Most tickers
have 0-1 splits in scope (2015+).

Usage:
    python scripts/bulk_splits.py
    python scripts/bulk_splits.py --tickers AAPL,NVDA,TSLA
    python scripts/bulk_splits.py --start 2010-01-01
"""
...
def _log(msg: str) -> None:
    print(f"[bulk_splits] {msg}", flush=True)
...
def main() -> int:
    ...
    from src.data.fetcher.corporate_actions import fetch_splits
    ...
    _log(f"Fetching splits for {len(tickers)} tickers, {args.start}..{end_date}")
    ...
    for i, ticker in enumerate(tickers, 1):
        t0 = time.time()
        try:
            df = fetch_splits(ticker, start_date=args.start, end_date=end_date,
    ...
    with sqlite3.connect(ds.db_path) as conn:
        summary = pd.read_sql(
            """SELECT COUNT(*) AS rows, COUNT(DISTINCT ticker) AS tickers,
                      MIN(date) AS min_d, MAX(date) AS max_d FROM stock_splits""",
            conn,
        )
    if not summary.empty:
        r = summary.iloc[0]
        _log(f"stock_splits: rows={r['rows']} tickers={r['tickers']} "
             f"{r['min_d']} .. {r['max_d']}")
