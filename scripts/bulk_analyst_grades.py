"""Bulk-fetch analyst grades (upgrades/downgrades) for the universe.

Step 4 Component 6 (2026-05-07). One call per ticker — `/grades` returns
all-history. UNIQUE(ticker, date, grading_company) handles re-runs idempotently.
Default save scope is 2015-01-01 → today.

Usage:
    python scripts/bulk_analyst_grades.py
    python scripts/bulk_analyst_grades.py --tickers AAPL,MSFT,NVDA
    python scripts/bulk_analyst_grades.py --start 2010-01-01
"""
...
def _log(msg: str) -> None:
    print(f"[bulk_analyst_grades] {msg}", flush=True)
...
def main() -> int:
    ...
    from src.data.fetcher.analyst import fetch_analyst_grades
    ...
    _log(f"Fetching grades for {len(tickers)} tickers, {args.start}..{end_date}")
    ...
    for i, ticker in enumerate(tickers, 1):
        t0 = time.time()
        try:
            df = fetch_analyst_grades(ticker, start_date=args.start, end_date=end_date,
            ...
            continue
        total += n
        _log(f"[{i}/{len(tickers)}] {ticker:<7} {n:>5} rows in DB  ({time.time()-t0:.1f}s)")
        if args.throttle_ms:
    ...
    with sqlite3.connect(ds.db_path) as conn:
        summary = pd.read_sql(
            """SELECT COUNT(*) AS rows, COUNT(DISTINCT ticker) AS tickers,
                      MIN(date) AS min_d, MAX(date) AS max_d FROM analyst_grades""",
            conn,
        )
    if not summary.empty:
        r = summary.iloc[0]
        _log(f"analyst_grades: rows={r['rows']:,} tickers={r['tickers']} "
             f"{r['min_d']} .. {r['max_d']}")
